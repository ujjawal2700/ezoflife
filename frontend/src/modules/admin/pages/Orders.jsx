...
const generateStatusHistory = (row) => {
  ...
  if (
    ...
    (!row.statusHistory || row.statusHistory.length <= 1)
  ) {
...
export default function Orders() {
  ...
  const getCustomerInvoiceCalculations = (order) => {
    const isCustomerRD = !!(
      order.customer?.gstNumber && order.customer?.gstNumber.trim()
    );
    const isVendorRD = !!(
      order.vendor?.gstNumber && order.vendor?.gstNumber.trim()
    );

    const serviceCost =
      order.grossServiceCost ||
      (order.items || []).reduce(
  ...
  const getVendorInvoiceCalculations = (order, customerCalc) => {
    ...
    const logisticsFee =
      order.orderType === "Walk-In"
        ...
        : order.priceBreakdown?.logisticsFee !== undefined
          ? order.priceBreakdown.logisticsFee
          : order.deliveryCharge || 50;

    const logisticsFeeGst = Math.round(logisticsFee * 0.18 * 100) / 100;

    const totalInvoiceAmt =
      platformFee + platformFeeGst + logisticsFee + logisticsFeeGst;
  ...
  const handleDownloadCustomerInvoice = (order) => {
    try {
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        `Invoice No: INV-CUST-${order.orderId || (order._id ? order._id.slice(-6).toUpperCase() : "TEMP")}`,
        200 - 15,
        26,
        { align: "right" },
      );
      ...
      doc.text(
        `Shop Name: ${order.vendor?.shopDetails?.name || "Spinzyt Partner"}`,
        15,
        62,
      );
      doc.text(`Phone: ${order.vendor?.phone || "N/A"}`, 15, 67);
      doc.text(
        `GSTIN: ${order.vendor?.gstNumber || "URD (Unregistered)"}`,
        15,
        72,
      );

      doc.text(
        `Name: ${order.customer?.displayName || "Valued Customer"}`,
        110,
        62,
      );
      doc.text(`Phone: ${order.customer?.phone || "N/A"}`, 110, 67);
      doc.text(
        `GSTIN: ${order.customer?.gstNumber || "URD (Individual)"}`,
        110,
        72,
      );
      ...
      const tableHeaders = [
        ["S.No", "Service Item", "Qty", "Rate (Rs.)", "Total (Rs.)"],
      ];
      ...
      doc.text(`${calc.serviceCost.toFixed(2)}`, 195, finalY, {
        align: "right",
      });

      doc.text("GST (18%):", 130, finalY + 6);
      doc.text(`${calc.gstAmount.toFixed(2)}`, 195, finalY + 6, {
        align: "right",
      });

      doc.setFont("helvetica", "bold");
      doc.text("Total Customer Payable:", 130, finalY + 13);
      doc.text(`${calc.totalInvoiceAmt.toFixed(2)}`, 195, finalY + 13, {
        align: "right",
      });
      ...
      doc.text(
        "Note: This is an electronically generated invoice raised by the Vendor to the Customer on Spinzyt.",
        15,
        finalY + 30,
      );

      doc.save(
        `Invoice_Customer_${order.orderId || (order._id ? order._id.slice(-6).toUpperCase() : "TEMP")}.pdf`,
      );
    } catch (err) {
  ...
  const handleDownloadVendorInvoice = (order) => {
    try {
      ...
      doc.setFont("helvetica", "bold");
      doc.setFontSize(14);
      doc.text("REVENUE & COMMISSIONS INVOICE", 200 - 15, 20, {
        align: "right",
      });
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        `Invoice No: INV-VEND-${order.orderId || (order._id ? order._id.slice(-6).toUpperCase() : "TEMP")}`,
        200 - 15,
        26,
        { align: "right" },
      );
      ...
      doc.text(`${customerCalc.totalInvoiceAmt.toFixed(2)}`, 195, finalY, {
        align: "right",
      });

      doc.text("Total Deductions (Inc. GST):", 120, finalY + 6);
      doc.text(`${calc.totalInvoiceAmt.toFixed(2)}`, 195, finalY + 6, {
        align: "right",
      });

      doc.setFont("helvetica", "bold");
      doc.text("Net Payout to Vendor:", 120, finalY + 13);
      doc.text(`${calc.netPaymentToVendor.toFixed(2)}`, 195, finalY + 13, {
        align: "right",
      });
      ...
      doc.setTextColor(100, 100, 100);
      doc.text(
        "GSTIN Reference: raised with Spinzyt GSTIN (23AAAAA1111A1Z1)",
        15,
        finalY + 25,
      );
      doc.text(
        "Note: This invoice records the platform and logistics commissions charged by Spinzyt to the Vendor.",
        15,
        finalY + 30,
      );

      doc.save(
        `Invoice_Vendor_${order.orderId || (order._id ? order._id.slice(-6).toUpperCase() : "TEMP")}.pdf`,
      );
    } catch (err) {
  ...
  const handleUpdateStatus = async (id, currentStatus) => {
    ...
    const newStatus = window.prompt(
      `Update status for order ${id}. Choices: ${nextStatuses.join(", ")}`,
      currentStatus,
    );
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const listForExport = [];
      const ordersToFlatten = Array.isArray(allMatching)
        ? allMatching
        : allMatching?.data || [];
      ...
      const tableRows = listForExport.map((row) => [
        ...
        "-",
        JSON.stringify(
          generateStatusHistory(row).map((h) => ({
            status: h.status,
            time: h.timestamp
        ...
        (() => {
          const completedEntry = generateStatusHistory(row).find(
            (h) => (h.status || "").toUpperCase() === "DELIVERED",
          );
          return completedEntry?.timestamp
            ? new Date(completedEntry.timestamp).toLocaleString("en-IN", {
                day: "2-digit",
                month: "short",
                year: "numeric",
                hour: "2-digit",
        ...
        `Rs. ${row.priceBreakdown?.platformFee !== undefined ? row.priceBreakdown.platformFee : row.ledger?.platformFee || 0}`,
        ...
        (() => {
          ...
          const platformGst =
            (row.priceBreakdown?.platformFee || 0) * (gstPercent / 100);
          const vendorGst =
            ((row.priceBreakdown?.baseWithArea || 0) +
              (row.priceBreakdown?.expressSurcharge || 0)) *
            (gstPercent / 100);
  ...
  const handleExportFile = async (format) => {
    ...
    try {
      ...
      const rows = listForExport.map((row) => [
        ...
        row.priceBreakdown?.platformFee !== undefined
          ? row.priceBreakdown.platformFee
          : row.ledger?.platformFee || 0,
        row.orderType === "Walk-In"
          ? row.deliveryCharge || 0
          : row.priceBreakdown?.logisticsFee !== undefined
            ? row.priceBreakdown.logisticsFee
            : row.deliveryCharge || 0,
        Math.round(
          (row.priceBreakdown?.platformFee || 0) *
            ((row.tier === "Heritage" ? 18 : 5) / 100),
        ),
        Math.round(
          ((row.priceBreakdown?.baseWithArea || 0) +
            (row.priceBreakdown?.expressSurcharge || 0)) *
      ...
      if (format === "excel") {
        XLSX.writeFile(
          wb,
          `EzofLife_Orders_Detail_${activeTab}_${new Date().getTime()}.xlsx`,
        );
      } else if (format === "csv") {
        XLSX.writeFile(
          wb,
          `EzofLife_Orders_Detail_${activeTab}_${new Date().getTime()}.csv`,
          { bookType: "csv" },
        );
      }
  ...
  const handleClearAllOrders = async () => {
    if (
      !window.confirm(
        "⚠️ WARNING: This will permanently delete ALL orders from the system. This action cannot be undone. Are you sure?",
      )
    )
      return;
  ...
  const uniqueZonesList = useMemo(() => {
    return backendServiceZones.map((name, idx) => ({
      _id: idx,
      zoneName: name,
    }));
  }, [backendServiceZones]);
    ...
    () =>
      [
        ...
        },
        {
          ...
          key: "orderId",
          render: (val, row) => (
            <span className="font-bold text-slate-900 text-[11px] tracking-[0.1em] uppercase group-hover:text-blue-600 transition-colors">
              {val}
            </span>
          ),
        },
        ...
        },
        {
          ...
          render: (val, row) => {
            const gstPercent = row.tier === "Heritage" ? 18 : 5;
            const vendorGst =
              ((val?.baseWithArea || 0) + (val?.expressSurcharge || 0)) *
              (gstPercent / 100);
            return (
              <span className="font-bold text-slate-900 tabular-nums text-xs">
      ...
      ].filter((col) => col.key !== "actions" || activeTab === "Completed"),
    [handleDownloadCustomerInvoice, handleDownloadVendorInvoice, activeTab],
  );
          ...
          <div className="bg-white rounded-lg p-6 shadow-2xl relative z-10 w-full max-w-lg border border-slate-100 flex flex-col gap-4 text-left">
            ...
            <div className="max-h-[60vh] overflow-y-auto bg-slate-50 p-4 border border-slate-200 rounded text-slate-800 flex flex-col gap-2">
              {selectedOrderItems.map((i, idx) => (
                <div
                  key={idx}
                  className="text-[11px] font-bold text-slate-700 border-b border-slate-200/50 pb-1.5 last:border-0 last:pb-0">
